...
test.describe("Cart Badge", () => {
	...
	test.describe("Count correctness", () => {
		test("display correct count after adding multiple products", async ({ productsPage }) => {
			const products = ["sauce-labs-backpack", "sauce-labs-bike-light", "sauce-labs-bolt-t-shirt"];
			for (const product of products) {
				await productsPage.clickAddToCartButtonFor(product);
			}
			await productsPage.header.cartBadge.expectToHaveText(String(products.length));
		});

		test("display correct count after removing multiple products", async ({ productsPage }) => {
			const products = ["sauce-labs-backpack", "sauce-labs-bike-light", "sauce-labs-bolt-t-shirt"];
			for (const product of products) {
				await productsPage.clickAddToCartButtonFor(product);
			}
			...
			await productsPage.header.cartBadge.expectToHaveText(String(products.length - 2));
		});
	...
	test.describe("State after page actions", () => {
		test("keep cart badge state after reloading page", async ({ productsPage }) => {
			const product = "sauce-labs-backpack";
			await productsPage.clickAddToCartButtonFor(product);
		...
		test("keep cart badge state after logout and login", async ({ productsPage, loginPage }) => {
			const product = "sauce-labs-backpack";
			await productsPage.clickAddToCartButtonFor(product);
